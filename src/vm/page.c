...
bool page_out (struct page *p) {
  ...
  if(dirty){
    // Is private?
      if(!p->private){
        // Si es hacemos write to back to file
        off_t goku = file_write_at(p->file, p->frame->base, p->file_bytes, p->file_offset);
        ok = (goku == p->file_bytes);
        if(ok){ p->frame = NULL; frame_free(p->frame); }
      }else{ // Sino es privado
        // Write back to swap
        ok = swap_out(p);
